        ...
        <div className="h-full flex flex-col gap-4 overflow-hidden">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">{remainingCount} remaining</p>
            </div>
          </div>

          <form onSubmit={handleAddTask} className="rounded-xl border bg-white p-4">
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1">
                <Label htmlFor="task-title" className="text-xs text-slate-500 font-medium">
                  Task
                </Label>
                <Input
                  id="task-title"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="flex flex-col gap-1">
                  <Label htmlFor="task-due" className="text-xs text-slate-500 font-medium">
                    Due date (optional)
                  </Label>
                  <Input
                    id="task-due"
              ...
              {!isAllDay && !!dueDate && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1">
                    <Label htmlFor="task-start" className="text-xs text-slate-500 font-medium">
                      Start time
                    </Label>
                    <Input
                      id="task-start"
                  ...
                  <div className="flex flex-col gap-1">
                    <Label htmlFor="task-end" className="text-xs text-slate-500 font-medium">
                      End time
                    </Label>
                    <Input
                      id="task-end"
          ...
          <div className="flex-1 overflow-y-auto rounded-xl border bg-white">
            ...
            <div className="divide-y">
              {tasks.length === 0 ? (
                <div className="p-4 text-sm text-slate-500">No tasks yet.</div>
              ) : (
                tasks.map((task) => (
